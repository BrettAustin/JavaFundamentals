...
public class SearchSortHomeworkDriver {
	...
	public static double sortedness(Comparable[] array) {
		int pairs = array.length-1;
		double valuePerPair = 1.0/pairs;
		double sortedNess = 1.0;
		int itemOne = 0;
		int itemTwo = 1;

		for(int i = 0; i< array.length-1;i++){
			if(array[itemOne].compareTo(array[itemTwo])>0){
				sortedNess = sortedNess - valuePerPair;
			}
			itemOne++;
			itemTwo++;
		}
	...
	public static double sortedness(Node<Comparable> node) {
		...
		if(temp!=null && temp.data!=null) { //determine length of chain
			while (temp != null) {
				lengthOfChain++;
				temp = temp.next;
			}
		...
		double valuePerPair = 1.0/(lengthOfChain-1);
		double sortedNess = 1.0;
		temp = node; //point it back to the beginning

		if(temp != null && temp.data!=null && temp.next != null){
			Node<Comparable> firstNode = node;
			Node<Comparable> secondNode = node.next;
