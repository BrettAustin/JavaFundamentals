...
public class HomeworkM11Driver {
	...
	public static void splice(QueueInterface firstQueue, QueueInterface secondQueue) {
		LinkedQueue<Object> temporaryQueue = new LinkedQueue<Object>();
		if ((firstQueue.isEmpty()==true)&&(secondQueue.isEmpty()==true)){
			 //do nothing
		}else if((firstQueue.isEmpty()==true)&&(secondQueue.isEmpty()==false)){ //1st is empty
			 while(!secondQueue.isEmpty()){
				 Object temp = null;
				 temp = secondQueue.getFront(); //get the front
				 firstQueue.enqueue(secondQueue.dequeue()); //take it off
				 temporaryQueue.enqueue(temp);//add to temp queue
			 }
			 while(!temporaryQueue.isEmpty()){//move it all back
				 Object temp = null;
				 secondQueue.enqueue(temporaryQueue.dequeue());
			 }

		}else if((firstQueue.isEmpty()==false)&&(secondQueue.isEmpty()==true)){ //2nd is empty
			//do nothing
		}else if((firstQueue.isEmpty()==false)&&(secondQueue.isEmpty()==false)) {//both have stuff
			while(!secondQueue.isEmpty()){
				Object temp = null;
				temp = secondQueue.getFront(); //get the front
				firstQueue.enqueue(secondQueue.dequeue()); //take it off
